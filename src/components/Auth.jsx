...
export default function Auth({ onAuth }) {
  const [isSignup, setIsSignup] = useState(false); // Start with Login like your screenshot
  ...
  const handle = async (e) => {
    ...
    const payload = isSignup
      ? { name: form.name.trim(), email: fakeEmail, age: Number(form.age), password: form.password, emergencyPhone: form.emergencyPhone.trim() }
      : { email: fakeEmail, password: form.password, name: form.name.trim(), age: Number(form.age) };
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: "1px solid rgba(255,255,255,0.2)", boxShadow: "0 20px 60px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.2)",
        position: "relative", zIndex: 2
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 24 }}>
          <span style={{ fontSize: 22 }}>✨</span>
          <span style={{ fontSize: 11, letterSpacing: "0.2em", color: "#fff", fontWeight: 700, opacity: 0.9 }}>WEBFLOW TRICKS AND WIZARDRY</span>
        </div>

        <h1 style={{ color: "#fff", fontSize: 30, fontWeight: 800, margin: "0 0 6px" }}>{isSignup ? "Create Account" : "Welcome Back"}</h1>
        <p style={{ color: "rgba(255,255,255,0.65)", fontSize: 13, marginBottom: 26 }}>{isSignup ? "Join MindGuard secure space" : "Log in to continue to your workspace"}</p>
        ...
        <div style={{ textAlign: "center", marginTop: 20, fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
        ...
        <div style={{ textAlign: "center", marginTop: 14, fontSize: 10, color: "rgba(255,255,255,0.45)", letterSpacing:"0.05em" }}>SECURE • FAST • MAGIC POWERED</div>
      </div>
    </div>
  );
}
...
const labelStyle = {
  fontSize: 11, color: "rgba(255,255,255,0.85)", background: "rgba(255,255,255,0.15)",
  padding: "4px 10px", borderRadius: 8, marginBottom: 6, display: "inline-block", fontWeight:600
};
const inputStyle = {
  width: "100%", padding: "13px 14px", borderRadius: 12, border: "1px solid rgba(255,255,255,0.22)",
  background: "rgba(255,255,255,0.14)", color: "#fff", outline: "none", boxSizing: "border-box", fontSize:14
};
const btnStyle = {
  marginTop: 6, width: "100%", padding: "13px", borderRadius: 14, border: "none",
  background: "linear-gradient(90deg, #6d28d9, #8b5cf6)", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer",
