...
export default function ThemeAvatarSettings({ onProfileUpdate }) {
  ...
  const [fontScale, setFontScale] = useState(
    () => localStorage.getItem("emovra_font_scale") || "100"
  );
  const [highContrast, setHighContrast] = useState(
    () => localStorage.getItem("emovra_high_contrast") === "true"
  );
  ...
  // FIX: this only ever cleared 'token'/'user' - mood history, the last
  // risk analysis, CBT worksheets, and the life timeline all persisted
  // indefinitely, unscoped to any user id. On a shared/school computer the
  // next person to use the app could read the previous student's mood/CBT/
  // crisis-analysis history after they believed they'd logged out.
  function logout() {
  ...
  const MOOD_COLORS = {
    ...
    Lonely: "#a78bfa",
    Overwhelmed: "#f472b6",
    "Don't Know What To Do": "#fbbf24",
    "Everything Fell On You At Once": "#f97316",
  };

  // FIX: this read the raw localStorage value directly and JSON.parse'd it,
  // but saveMood()/storage.js writes this key through encryptLocal() (XOR +
  // base64, not valid JSON) - so JSON.parse always threw, was swallowed by
  // this catch, and the mood ring silently showed nothing for every user
  // with real mood history. Now decrypts first, same as storage.js's own
  // loadMoodHistory().
  function moodRingColor() {
    try {
      const raw = localStorage.getItem("mental_health_mood_history");
      if (!raw) return null;
      const history = JSON.parse(decryptLocal(raw));
  ...
  function loadProfile() {
    setLoadError("");
    Promise.all([
      fetch(`${API}/profile/options`).then((r) => {
        if (!r.ok) throw { status: r.status, source: "options" };
        return r.json();
      }),
      fetch(`${API}/profile/me`, { headers: authHeaders() }).then((r) => {
        if (!r.ok) throw { status: r.status, source: "profile" };
        return r.json();
      }),
    ])
      .then(([optionsData, profileData]) => {
        ...
        if (profileData.success) {
          ...
          setCustomAccent(
            profileData.customTheme?.accent || profileData.theme?.accent || "#d4b07a"
          );
        }
    ...
    >
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, opacity: 0.6, marginTop: 4 }}>
            {t("themeAvatarSettings.subtitle")}
          </p>
        </div>
        <button
          onClick={logout}
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
          {t("themeAvatarSettings.themeLabel")}
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {options.themes.map((th) => (
            <button
              key={th.id}
              onClick={() => {
                applyThemeVars({ ...th, backgroundImage: current.backgroundImage });
                setCustomBg(th.bg);
                setCustomCard(th.card);
                setCustomAccent(th.accent);
                save({ themePreference: th.id });
              }}
              style={{
                display: "flex",
                alignItems: "center",
                ...
                cursor: "pointer",
                border:
                  !isCustomActive && current.theme?.id === th.id
                    ? "2px solid #d4b07a"
      ...
      <div
        style={{ marginTop: 20, padding: 16, borderRadius: 12, border: "1px solid var(--border)" }}
      >
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
          🖌 {t("themeAvatarSettings.customThemeHeading")}
        </div>
        <p style={{ fontSize: 11, opacity: 0.6, margin: "0 0 12px" }}>
          {t("themeAvatarSettings.customThemeSubtitle")}
        </p>

        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
          <HSLColorPicker
            label={t("themeAvatarSettings.backgroundColorLabel")}
            value={customBg}
            onChange={(hex) => {
              setCustomBg(hex);
              previewCustomThrottled({ bg: hex });
            }}
          />
          <HSLColorPicker
            label={t("themeAvatarSettings.cardColorLabel")}
            value={customCard}
            onChange={(hex) => {
              setCustomCard(hex);
              previewCustomThrottled({ card: hex });
            }}
          />
          <HSLColorPicker
            label={t("themeAvatarSettings.accentColorLabel")}
            value={customAccent}
            onChange={(hex) => {
              setCustomAccent(hex);
              previewCustomThrottled({ accent: hex });
            }}
          />
        </div>
        ...
        >
          {savingCustom
            ? t("themeAvatarSettings.savingCustomTheme")
            : t("themeAvatarSettings.saveCustomThemeButton")}
        </button>
      </div>
      ...
      >
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
          🖼️ {t("themeAvatarSettings.backgroundImageHeading")}
        </div>
        <p style={{ fontSize: 11, opacity: 0.6, margin: "0 0 12px" }}>
          {t("themeAvatarSettings.backgroundImageSubtitle")}
        </p>
        ...
        {current.backgroundImage && !bgPreviewImage && (
          <div style={{ marginBottom: 14 }}>
            <img
              ...
              alt={t("themeAvatarSettings.currentBackgroundAlt")}
            ...
            >
              {removingBg
                ? t("themeAvatarSettings.removingBackground")
                : t("themeAvatarSettings.removeBackgroundImageButton")}
            </button>
          </div>
        )}
            ...
            <div style={{ flex: 1, minWidth: 220 }}>
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button
                  ...
                  disabled={!bgPrompt.trim() || generatingImage}
                  style={{
                    background: "var(--accent)",
                    color: "#000",
                    border: "none",
                    ...
                    fontWeight: 700,
                    fontSize: 12,
                    cursor: bgPrompt.trim() ? "pointer" : "not-allowed",
                    opacity: bgPrompt.trim() ? 1 : 0.5,
                    whiteSpace: "nowrap",
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 6,
                ...
                >
                  ...
                  {generatingImage ? (
                    t("themeAvatarSettings.generatingImage")
                  ) : (
                    <>✨ {t("themeAvatarSettings.generateButton")}</>
                  )}
                </button>
              </div>
              <p style={{ fontSize: 10, opacity: 0.5, marginTop: 4 }}>
                {t("themeAvatarSettings.generateNote")}
              </p>
            </div>
          </div>
        )}
        ...
        {bgPreviewImage && (
          <div>
            <img
              src={bgPreviewImage}
              alt={t("themeAvatarSettings.previewAlt")}
              ...
              {bgExtracted ? (
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 11, opacity: 0.6 }}>
                    {t("themeAvatarSettings.matchedColors")}
                  </span>
                  <span
                    style={{
                      width: 20,
                      height: 20,
                      borderRadius: 6,
                      background: bgExtracted.bg,
                      borderWidth: 1,
                      borderStyle: "solid",
                      borderColor: "var(--border)",
                    }}
                    title={t("themeAvatarSettings.backgroundColorLabel")}
                  />
                  <span
                    style={{
                      width: 20,
                      height: 20,
                      borderRadius: 6,
                      background: bgExtracted.card,
                      borderWidth: 1,
                      borderStyle: "solid",
                      borderColor: "var(--border)",
                    }}
                    title={t("themeAvatarSettings.swatchCardTitle")}
                  />
                  <span
                    style={{
                      width: 20,
                      height: 20,
                      borderRadius: 6,
                      background: bgExtracted.accent,
                      borderWidth: 1,
                      borderStyle: "solid",
                      borderColor: "var(--border)",
                    }}
                    title={t("themeAvatarSettings.swatchAccentTitle")}
                  />
                </div>
              ) : (
                <p style={{ fontSize: 11, opacity: 0.6 }}>
                  {t("themeAvatarSettings.colorMatchFailed")}
                </p>
              )}
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
              ...
              >
                {applyingBg
                  ? t("themeAvatarSettings.applyingBackground")
                  : t("themeAvatarSettings.useThisBackgroundButton")}
              </button>
              <button
                onClick={cancelBgPreview}
                style={{
                  background: "transparent",
                  border: "1px solid var(--border)",
                  color: "var(--text)",
      ...
      <div style={{ marginTop: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
          {t("themeAvatarSettings.avatarLabel")}
        </div>
        ...
        <div style={{ marginBottom: 12, display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative", width: 60, height: 60 }}>
            <div
              style={{
                ...
                borderRadius: "50%",
                overflow: "hidden",
                border: `2px solid ${moodRingColor() || "var(--accent)"}`,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 28,
            ...
            >
              {current.avatarType === "custom" && current.avatarImage ? (
                <img
                  ...
                  alt={t("themeAvatarSettings.yourAvatarAlt")}
          ...
          >
            {uploading
              ? t("themeAvatarSettings.uploadingPhoto")
              : t("themeAvatarSettings.uploadYourPhotoButton")}
            <input
              type="file"
              accept="image/png,image/jpeg,image/webp"
        ...
        {uploadError && (
          <p style={{ fontSize: 11, color: "#f87171", marginBottom: 10 }}>{uploadError}</p>
        )}
          ...
          {options.avatars.map((a) => (
            <button
              ...
              style={{
                ...
                borderRadius: "50%",
                border:
                  current.avatarType !== "custom" && current.avatar === a
                    ? "2px solid #d4b07a"
                    : "1px solid var(--border)",
                background: "transparent",
                cursor: "pointer",
        ...
        {options.accessories?.length > 0 && (
          <div style={{ marginTop: 14 }}>
            <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 8 }}>
              {t("themeAvatarSettings.accessoriesHeading")}
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <button
                onClick={() => save({ avatarAccessory: "" })}
                style={{
                  fontSize: 11,
                  padding: "8px 12px",
                  borderRadius: 999,
                  border: !current.avatarAccessory
                    ? "2px solid #d4b07a"
                    : "1px solid var(--border)",
                  background: "transparent",
                  color: "var(--text)",
                ...
                return (
                  <button
                    ...
                    title={
                      unlocked
                        ? acc.name
                        : t("themeAvatarSettings.unlocksAtLevel", { level: acc.minLevel })
                    }
                    style={{
                      fontSize: 18,
                      padding: "8px 12px",
                      borderRadius: 999,
                      border:
                        current.avatarAccessory === acc.emoji
                          ? "2px solid #d4b07a"
                          : "1px solid var(--border)",
                      background: "transparent",
                      color: "var(--text)",
                  ...
                  >
                    ...
                    {!unlocked && (
                      <span style={{ fontSize: 9 }}>
                        {t("themeAvatarSettings.lockedLevelBadge", { level: acc.minLevel })}
                      </span>
                    )}
                  </button>
                );
      ...
      <div style={{ marginTop: 20 }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
          {t("themeAvatarSettings.birthdayHeading")}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <select
            defaultValue={current.birthdayMonth || ""}
            onChange={(e) =>
              save({ birthdayMonth: Number(e.target.value), birthdayDay: current.birthdayDay || 1 })
            }
            style={{
              padding: 8,
              borderRadius: 8,
          ...
          <select
            defaultValue={current.birthdayDay || ""}
            onChange={(e) =>
              save({
                birthdayDay: Number(e.target.value),
                birthdayMonth: current.birthdayMonth || 1,
              })
            }
            style={{
              padding: 8,
      ...
      {pushSupported && (
        <div style={{ marginTop: 28, paddingTop: 20, borderTop: "1px solid var(--border)" }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>
            {t("themeAvatarSettings.notificationsHeading")}
          </div>
          <p style={{ fontSize: 11, opacity: 0.6, marginBottom: 10 }}>
            {t("themeAvatarSettings.notificationsSubtitle")}
          </p>
          <button
            ...
            style={{
              ...
              background: pushSubscribed ? "transparent" : "var(--accent)",
              color: pushSubscribed ? "var(--text)" : "#000",
              fontWeight: 700,
              cursor: "pointer",
              fontSize: 12,
            }}
          >
            {pushLoading
              ? t("themeAvatarSettings.pushWorking")
              : pushSubscribed
                ? t("themeAvatarSettings.turnOffNotificationsButton")
                : t("themeAvatarSettings.enableDailyReminderButton")}
          </button>
      ...
      <div style={{ marginTop: 28, paddingTop: 20, borderTop: "1px solid var(--border)" }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>
          {t("themeAvatarSettings.accessibilityHeading")}
        </div>
        <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 8 }}>
          {t("themeAvatarSettings.textSizeLabel")}
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          {[
            { v: "90", l: t("themeAvatarSettings.fontSizeSmall") },
            { v: "100", l: t("themeAvatarSettings.fontSizeDefault") },
            { v: "115", l: t("themeAvatarSettings.fontSizeLarge") },
            { v: "130", l: t("themeAvatarSettings.fontSizeExtraLarge") },
          ].map((opt) => (
            <button
              key={opt.v}
              ...
              style={{
                ...
                fontSize: 12,
                cursor: "pointer",
                border: fontScale === opt.v ? "2px solid var(--accent)" : "1px solid var(--border)",
                background: fontScale === opt.v ? "rgba(212,176,122,0.15)" : "transparent",
                color: "var(--text)",
              }}
            >
              {opt.l}
        ...
        <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            style={{
              ...
              fontSize: 12,
              cursor: "pointer",
              border: highContrast ? "2px solid var(--accent)" : "1px solid var(--border)",
              background: highContrast ? "rgba(212,176,122,0.15)" : "transparent",
              color: "var(--text)",
            }}
          >
            {highContrast ? (
              <>✓ {t("themeAvatarSettings.highContrastOn")}</>
            ) : (
              t("themeAvatarSettings.highContrastMode")
            )}
          </button>
        </div>
      </div>
      ...
      <div style={{ marginTop: 28, paddingTop: 20, borderTop: "1px solid var(--border)" }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10 }}>
          {t("themeAvatarSettings.yourDataHeading")}
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button
            onClick={exportData}
          ...
          >
            {exporting ? (
              t("themeAvatarSettings.preparingExport")
            ) : (
              <>⬇ {t("themeAvatarSettings.exportMyDataButton")}</>
            )}
          </button>
          <button
        ...
        {showDeleteConfirm && (
          ...
          >
            <p style={{ fontSize: 13, margin: "0 0 10px" }}>
              {t("themeAvatarSettings.deleteWarningIntro")}{" "}
              <b>{t("themeAvatarSettings.deleteWarningUndone")}</b>{" "}
              {t("themeAvatarSettings.deleteWarningType", { word: "DELETE" })}
            </p>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <input
                value={deleteConfirmText}
                onChange={(e) => setDeleteConfirmText(e.target.value)}
                placeholder={t("themeAvatarSettings.deleteConfirmPlaceholder", { word: "DELETE" })}
                style={{
                  flex: 1,
                  minWidth: 140,
                  padding: "8px 12px",
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: "none",
                  background: deleteConfirmText === "DELETE" ? "#dc2626" : "#555",
                  color: "#fff",
                  fontWeight: 700,
              ...
              >
                {deleting
                  ? t("themeAvatarSettings.deletingButton")
                  : t("themeAvatarSettings.permanentlyDeleteButton")}
              </button>
              <button
                onClick={() => {
      ...
      <div style={{ marginTop: 32, paddingTop: 20, borderTop: "1px solid var(--border)" }}>
        <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
          💬 {t("themeAvatarSettings.feedbackHeading")}
        </div>
        <p style={{ fontSize: 11, opacity: 0.6, margin: "0 0 10px" }}>
          {t("themeAvatarSettings.feedbackSubtitle")}
        </p>
        <textarea
          value={feedbackText}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 8 }}>
          ...
          >
            {feedbackStatus === "sending"
              ? t("themeAvatarSettings.sendingFeedback")
              : t("themeAvatarSettings.sendFeedbackButton")}
          </button>
          {feedbackStatus === "sent" && (
            <span style={{ fontSize: 12, color: "#4ade80" }}>
              ✓ {t("themeAvatarSettings.feedbackSent")}
            </span>
          )}
          {feedbackStatus === "error" && (
            <span style={{ fontSize: 12, color: "#f87171" }}>
              {t("themeAvatarSettings.feedbackError")}
            </span>
          )}
        </div>
      </div>
