...
export default function VoiceToneAnalyzer({ onResult, token }) {
  ...
  function stop() {
    ...
    const out = {
      ...
      isCrisis,
      helpline: isCrisis? t("voiceToneAnalyzer.helplineText", { teleManas: "14416", kiran: "1800-599-0019" }) : null,
      advice: isCrisis? t("voiceToneAnalyzer.crisisAdvice", { number: "14416" }) : wordAnalysis?.advice || t("voiceToneAnalyzer.adviceStable"),
  ...
  return (
    <div style={{ maxWidth: 700, width: "100%", background: "var(--card-bg, #fff)", border: "1px solid var(--border, #ddd)", borderRadius: 12, padding: 16, textAlign: "left" }}>
      ...
      <p style={{ fontSize: 12, opacity: 0.7, margin: "6px 0" }}>{t("voiceToneAnalyzer.subtitle")}</p>
      {error && <div style={{ background: "#fee2e2", color: "#991b1b", padding: 8, borderRadius: 8, fontSize: 12, marginTop: 8 }}>{error}</div>}
      <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 12, flexWrap: "wrap" }}>
        {!recording? <button onClick={start} className="primary-btn" style={{ background: "#E7D3A3", padding: "10px 20px", borderRadius: 20, border: "none", cursor: "pointer", fontWeight: 600 }}>{t("voiceToneAnalyzer.startButton")}</button> : <button onClick={stop} className="primary-btn" style={{ background: "#dc2626", color: "#fff", padding: "10px 20px", borderRadius: 20, border: "none", cursor: "pointer" }}>{t("voiceToneAnalyzer.stopButton", { time: fmt(timer) })}</button>}
        {recording && <span style={{ fontWeight: 800, color: "#dc2626" }}>{t("voiceToneAnalyzer.recStatus", { time: fmt(timer), transcript: transcript.slice(-40) })}</span>}
        {aiLoading && <span style={{ fontSize: 12, color: "#6b7280" }}>{t("voiceToneAnalyzer.geminiAnalyzing")}</span>}
      </div>
      {transcript && <div style={{ marginTop: 10, padding: 8, background: "#f3f4f6", borderRadius: 6, fontSize: 13 }}><b>{t("voiceToneAnalyzer.heardLabel")}</b> {transcript}</div>}
      ...
      {result && (
        <div style={{ marginTop: 14, border: `2px solid ${color}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{ background: color, color: "white", padding: "10px 14px", fontWeight: 800, display: "flex", justifyContent: "space-between" }}>
            <span>{t("voiceToneAnalyzer.riskLabel", { level: levelLabel(t, result.level), aiTag: result.isAI? t("voiceToneAnalyzer.geminiTag") : "" })}</span><span>{t("voiceToneAnalyzer.scoreLabel", { score: result.score })}</span>
          </div>
          <div style={{ padding: 12, background: "#f9fafb", fontSize: 13, lineHeight: 1.7 }}>
            <div><b>{t("voiceToneAnalyzer.wordsLabel")}</b> {result.transcript}</div>
            <div><b>{t("voiceToneAnalyzer.emotionLabel")}</b> {emotionLabel(t, result.emotion)} | <b>{t("voiceToneAnalyzer.pauseLabel")}</b> {result.pauseRatio}% | <b>{t("voiceToneAnalyzer.wordScoreLabel")}</b> {result.wordScore}</div>
            <div><b>{t("voiceToneAnalyzer.reasonsLabel")}</b> {result.reasons.join(" | ")}</div>
            {result.saved && <div style={{ fontSize: 11, color: "#16a34a", marginTop: 4 }}>{t("voiceToneAnalyzer.encryptedSaved")}</div>}
            <div style={{ marginTop: 8, padding: 10, background: "white", borderRadius: 8, border: "1px solid #e5e7eb" }}>
              ...
              {result.isCrisis && <div style={{ marginTop: 8, padding: 8, background: "#fee2e2", borderRadius: 6, color: "#991b1b", fontWeight: 700 }}> {result.helpline}{t("voiceToneAnalyzer.safetyFirstSuffix")}</div>}
            </div>
          </div>
        </div>
      )}
