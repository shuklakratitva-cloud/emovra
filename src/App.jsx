...
export default function App() {
  ...
  const handleAnalyze = () => {
    ...
    const analysisText = cleanText.length > 1000 
      ? cleanText.slice(0, 500) + " " + cleanText.slice(-500)
      : cleanText;

    setTimeout(() => {
      const sentiment = analyzeSentiment(analysisText);
      const emotion = detectEmotion(analysisText);
      const risk = calculateRisk(analysisText, sentiment, emotion);
      
      const newResult = { text: cleanText, ...sentiment, ...emotion, ...risk, id: Date.now(), time: new Date().toLocaleString() };
      setResult(newResult);
      addMood(newResult);
      setLoading(false);
    }, 400);
        ...
        <main className="container" style={{paddingTop:24, paddingBottom:20}}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, alignItems:'start'}}>
            <div style={{padding:20, border:'1px solid var(--border)', borderRadius:16, background:'var(--card-bg)'}}>
              ...
              />
              <div style={{textAlign:'right', fontSize:12, opacity:0.5, marginTop:4}}>{text.length} / 2000</div>
          ...
          <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:20, marginTop:20}}>
            <div style={{padding:16, border:'1px solid var(--border)', borderRadius:16, background:'var(--card-bg)'}}>
              ...
              <p style={{fontSize:12, opacity:0.7, marginTop:6}}>India's national mental health support service.</p>
            </div>
            ...
            <div style={{padding:16, border:'1px solid var(--border)', borderRadius:16, background:'var(--card-bg)'}}>
              <strong style={{fontSize:13}}>⚠ Emergency Notice</strong>
              <p style={{fontSize:11, opacity:0.7, marginTop:6}}>If you are in immediate danger, please contact emergency services immediately.</p>
            </div>
          </div>
